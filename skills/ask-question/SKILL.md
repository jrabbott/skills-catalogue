...
3. Treat the server as available only if that probe succeeds.
...
#### If the filesystem MCP server is not available

1. Tell the user that `@modelcontextprotocol/server-filesystem` is not available in this session.
...
2. Search each folder listed in `documentation_folders`.
...
3. You confirm that the filesystem MCP server is available.
...
- Stop when the filesystem MCP server is missing and the user chooses stop.
